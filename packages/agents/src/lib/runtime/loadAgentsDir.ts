...
export async function loadAgentsDir(dir: string): Promise<AgentProfile[]> {
  if (!existsSync(dir)) {
    agentsLog.debug('loadAgentsDir skip (no dir)', { dir });
    return [];
  }
  ...
  try {
    ...
    agentsLog.warn('loadAgentsDir readdir failed', {
      dir,
    ...
    try {
      ...
      for (const entry of list) {
        if (isAgentProfile(entry)) {
          ...
          agentsLog.warn('loadAgentsDir skip invalid entry', {
            file,
      ...
      agentsLog.warn('loadAgentsDir parse failed', {
        file,
        error: err instanceof Error ? err.message : String(err),
      });
    }
  ...
  agentsLog.info('loadAgentsDir', { dir, profileCount: profiles.length });
  return profiles;
}
