/**
 * Tiny structured logger for the agents package.
 *
 * Every message is prefixed with `[Agents]` so operators can grep the
 * backend stdout for the whole agent-tool lifecycle (TodoWrite,
 * EnterPlanMode, AgentTool, Task*, sub-agent events, plan-broker
 * resolution, HTTP routes). Level-gated by the `PIPEFX_AGENTS_LOG`
 ...
 *   "silent"                    → nothing
 *
 * Keep payloads short — this is an operational log, not a transcript.
 * Objects are stringified with a size cap so a giant tool response
 * doesn't blow past terminal width.
 */
...
function emit(
  ...
  const line = `[Agents] ${level.toUpperCase()} ${event}${formatContext(ctx)}`;
  if (level === 'error') {
    console.error(line);
  } else if (level === 'warn') {
    console.warn(line);
