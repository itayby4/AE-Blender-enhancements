...
export const klingProvider: VideoProvider = {
  ...
  async generate(prompt: string, options?: VideoOptions): Promise<{ id: string; status: string; url?: string }> {
    const { imageRef, imageTailRef, duration = '5', resolution = '720p', aspectRatio = '16:9' } = options || {};
    ...
    if (imageTailRef) {
      if (imageTailRef.startsWith('http') || imageTailRef.startsWith('https')) {
        ...
        const base64DataTail = imageTailRef.includes('base64,') ? imageTailRef.split('base64,')[1] : imageTailRef;
    ...
    if (data.code !== 0) {
      throw new Error(`Kling API Error ${data.code}: ${data.message || JSON.stringify(data)}`);
    }
