...
> = {
  ...
  unknown: {
    ...
    color: 'text-gray-400',
    bg: 'bg-gray-500/10',
    border: 'border-gray-500/50 hover:border-gray-500',
  },
};
...
export function MediaNode({
  ...
  if (isCompact) {
    ...
    return (
      ...
      >
        <div
          className={`absolute bottom-0 left-0 right-0 h-4 opacity-80 ${bottomColor}`}
        />
        <Handle
          type="target"
  ...
  return (
    <div
      className={`relative w-64 ${
        isExpanded
          ? 'scale-[1.5] origin-center shadow-[0_0_30px_rgba(0,200,255,0.3)] z-50'
          : ''
      }`}
    >
      {/* Input Handle */}
      ...
      />
      <Card
        className={`shadow-xl bg-card/95 backdrop-blur-md border-2 overflow-hidden transition-all duration-300 hover:shadow-2xl ${style.border} w-full h-full`}
      >
        {/* Header */}
        <CardHeader
          className={`p-2.5 pb-2 border-b border-border/50 ${style.bg}`}
        >
          <CardTitle
            className={`font-bold flex items-center gap-2 ${style.color} text-sm`}
          >
