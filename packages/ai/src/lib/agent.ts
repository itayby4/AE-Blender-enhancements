...
export function createAgent(config: AgentConfig): Agent {
  ...
  return {
    async chat(message: string, options?: ChatOptions): Promise<string> {
      ...
      const isSubtitleAllowed = !options?.allowedTools || options.allowedTools.includes('auto_generate_hebrew_subtitles');
      if (isSubtitleAllowed) {
        geminiTools.push({
          functionDeclarations: [{
            name: 'auto_generate_hebrew_subtitles',
            description: 'Automatically extracts audio from DaVinci Resolve, transcribes it via Whisper, translates it to Hebrew via GPT-4o, and inserts the subtitles back into the timeline as an SRT. Call this if the user asks for Hebrew subtitles.',
            parameters: {
              type: 'OBJECT',
              properties: {
      ...
      while (response.functionCalls && response.functionCalls.length > 0) {
        ...
        try {
          ...
          if (callName === 'auto_generate_hebrew_subtitles') {
            const args = (call.args as Record<string, unknown>) ?? {};
            console.log("Running Backend Pipeline: auto_generate_hebrew_subtitles", args);
            
            // 1. Trigger audio rendering
            const renderResult = await config.registry.callTool('render_timeline_audio', args);
            ...
            try {
              ...
              if (resJson.audio_chunks && Array.isArray(resJson.audio_chunks)) {
                ...
                if (allSegments.length > 0) {
                  console.log(`Translation completed! Adding ${allSegments.length} segments to timeline...`);
                  try {
                   ...
                   contentStr = JSON.stringify({ error: "No segments could be transcribed or the audio was silent." });
                }
              } else if (resJson.error) {
                contentStr = JSON.stringify({ error: resJson.error });
              }
            } catch (e) {
               contentStr = JSON.stringify({ error: "Failed to parse render_timeline_audio output." });
            }
            ...
            contentStr = Array.isArray(result.content) 
