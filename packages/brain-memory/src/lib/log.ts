...
function shorten(v: unknown): string {
  ...
  try {
    ...
    return json.length <= MAX_VALUE_LEN ? json : `${json.slice(0, MAX_VALUE_LEN)}…`;
  } catch {
    return String(v);
  }
}
...
function emit(level: LogLevel, event: string, ctx?: Record<string, unknown>): void {
  ...
  const line = `[Brain-Memory] ${level.toUpperCase()} ${event}${formatContext(ctx)}`;
  if (level === 'error') {
    console.error(line);
  } else if (level === 'warn') {
    console.warn(line);
