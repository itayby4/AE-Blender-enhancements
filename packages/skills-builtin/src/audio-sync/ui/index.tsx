// ── @pipefx/skills-builtin — Audio Sync bundled UI ───────────────────────
// Phase 12.11: the A/V Sync Studio (formerly
// `apps/desktop/src/features/audio-sync/AudioSyncDashboard.tsx`) ships as
// a `component`-mode skill, mirroring the 12.10 Subtitles pilot.
//
// The component owns its workflow + state and posts directly to
// `POST http://localhost:3001/api/audio-sync/run`. Skill inputs declared
// in SKILL.md are intentionally empty — the user interaction (file
// picker, results list) is fully in the bundled UI. The runner just
// emits the mount instruction.
...
const STAGE_LABELS: Record<PipelineStage, string> = {
  idle: 'Ready',
  exporting: 'Exporting timeline XML…',
  discovering: 'Discovering video sources…',
  correlating: 'Running FFT cross-correlation…',
  injecting: 'Injecting synced audio into XML…',
  importing: 'Importing synced timeline…',
...
// host-injected at mount. We accept `getToken` so the workflow fetches
// can attach the host's Bearer JWT and clear the backend's auth gate.
export interface AudioSyncSkillProps {
  getToken?: () => Promise<string | null>;
}

export default function AudioSyncSkill({ getToken }: AudioSyncSkillProps = {}) {
  ...
  const handleSync = async () => {
    ...
    try {
      ...
      if (data.sync_map) {
        const results: SyncResult[] = Object.entries(
          data.sync_map as Record<string, unknown>
        ).flatMap(([videoPath, matches]) => {
          const videoName = videoPath.split(/[\\/]/).pop() || videoPath;
          const items = Array.isArray(matches) ? matches : [matches];
          return items.map((info: { audio_path: string; offset_seconds: number }) => ({
            video: videoName,
