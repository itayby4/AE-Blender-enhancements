// ── @pipefx/skills-builtin — Subtitles bundled UI ────────────────────────
// Phase 12.10 pilot: the Subtitles dashboard (formerly
// `apps/desktop/src/features/subtitles/SubtitlesDashboard.tsx`) ships as a
// `component`-mode skill. Mounted by `BundledSkillHost` against the
// registry seeded in `apps/desktop/src/app/bundled-skill-registry.ts`.
//
// The component hits `POST http://localhost:3001/api/subtitles/generate`
// directly — same backend route as before. The runner mints a
// `SkillMountInstruction` only; it does NOT drive the workflow, so this
// component owns its own state and lifecycle. No skill inputs are
// declared in the SKILL.md frontmatter, so `inputs` arrives empty.
...
import { Card, CardContent } from '../../_ui/card.js';
import { Label } from '../../_ui/label.js';
import { Input } from '../../_ui/input.js';
import { Slider } from '../../_ui/slider.js';
import { Switch } from '../../_ui/switch.js';
import {
  Select,
  SelectContent,
  SelectGroup,
...
// host-injected at mount. This component owns its workflow + state and
// ignores everything except `getToken` — the desktop's auth gate at
// `apps/backend/main.ts:417` requires a Bearer JWT on every backend call,
// so the workflow fetches forward whatever the host's auth layer hands
// back (typically `getAccessToken` from `@pipefx/auth/ui`).
export interface SubtitlesSkillProps {
  getToken?: () => Promise<string | null>;
}

export default function SubtitlesSkill({ getToken }: SubtitlesSkillProps = {}) {
  const [language, setLanguage] = useState('');
  const [customLanguage, setCustomLanguage] = useState('');
  const [maxWords, setMaxWords] = useState(5);
  const [maxChars, setMaxChars] = useState(25);
  const [useMaxChars, setUseMaxChars] = useState(true);
  const [maxCharsPerLine, setMaxCharsPerLine] = useState(15);
  const [useMaxCharsPerLine, setUseMaxCharsPerLine] = useState(true);
      ...
      const response = await fetch(
        'http://localhost:3001/api/subtitles/generate',
        {
          ...
          body: JSON.stringify({
            ...
            max_words_per_chunk: maxWords,
            max_chars_per_chunk: useMaxChars ? maxChars : undefined,
            max_chars_per_line: useMaxCharsPerLine ? maxCharsPerLine : undefined,
          ...
          <div className="p-4 md:p-6 flex flex-col gap-4 md:gap-5">
            ...
            <Card className="shadow-sm border-border/50">
              <CardContent className="p-4 space-y-5">
                ...
                {/* Characters Limit */}
                <div className={`space-y-3 pt-3 border-t border-border/40 transition-opacity ${!useMaxChars ? 'opacity-50' : ''}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       <Label className="text-sm">Max Characters</Label>
                       <Switch
                         id="subtitle-use-max-chars"
                         checked={useMaxChars}
                         onCheckedChange={setUseMaxChars}
                         className="scale-75 origin-left"
                       />
                    </div>
                    <span className="text-sm font-bold tabular-nums bg-muted px-2 py-0.5 rounded min-w-[32px] text-center">
                      {useMaxChars ? maxChars : 'Off'}
                    </span>
                  </div>
                  <Slider
                    id="subtitle-max-chars"
                    min={10}
                    max={80}
                    step={5}
                    value={[maxChars]}
                ...
                {/* Line Break Character Limit */}
                <div className={`space-y-3 pt-3 border-t border-border/40 transition-opacity ${!useMaxCharsPerLine ? 'opacity-50' : ''}`}>
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2">
                       <Label className="text-sm">Line Break After</Label>
                       <Switch
                         id="subtitle-use-max-chars-per-line"
                         checked={useMaxCharsPerLine}
                         onCheckedChange={setUseMaxCharsPerLine}
                         className="scale-75 origin-left"
                       />
                    </div>
                    <span className="text-sm font-bold tabular-nums bg-muted px-2 py-0.5 rounded min-w-[32px] text-center">
                      {useMaxCharsPerLine ? maxCharsPerLine : 'Off'}
                    </span>
                  </div>
                  <Slider
                    id="subtitle-max-chars-per-line"
                    min={5}
                    max={50}
                    step={1}
                    ...
                    onValueChange={(val) => {
                      const arr = Array.isArray(val) ? val : [val];
                      setMaxCharsPerLine(arr[0]);
                    }}
                    disabled={!useMaxCharsPerLine}
                  />
                  <div className="flex justify-between text-[10px] text-muted-foreground font-mono">
                    <span>5</span>
                    <span>50</span>
