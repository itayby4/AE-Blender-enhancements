...
import { MAX_CONCURRENT_SUBAGENTS } from '@pipefx/brain-contracts';
import type { Agent, ChatOptions } from '@pipefx/agent-loop-kernel';
import type { AgentConfig } from '@pipefx/brain-loop';
import { createAgent, WORKER_SYSTEM_PROMPT } from '@pipefx/brain-loop';
import type {
  AgentSessionStore,
  TaskOutputStore,
  TaskTranscriptStore,
} from '@pipefx/brain-tasks';
import { createTaskTranscriptStore, generateTaskId } from '@pipefx/brain-tasks';
import { brainSubagentsLog } from '../log.js';

export type { SubAgentEvent, RunSubAgentOptions, ForkSubAgentOptions };
...
export interface SubAgentRuntime extends SubAgentsApi {
...
function resolveAgentSelection(
  ...
  if (agentName) {
    ...
    if (!profile) {
      brainSubagentsLog.warn('agent profile not found', { agentName });
    } else {
      return {
        systemPrompt:
  ...
  async function drive(opts: {
    ...
    const chatOpts: ChatOptions = {
      ...
      onToolCallStart: (name, args) => {
        brainSubagentsLog.debug('sub-agent tool_start', { taskId, tool: name });
        onEvent?.({ type: 'tool_start', taskId, name, args });
      },
      onToolCallComplete: (name, _result, err) => {
        if (err) {
          brainSubagentsLog.warn('sub-agent tool_error', {
            taskId,
            tool: name,
            error: err.message,
          });
        } else {
          brainSubagentsLog.debug('sub-agent tool_done', { taskId, tool: name });
        }
    ...
    try {
      ...
      brainSubagentsLog.info('sub-agent done', {
        sessionId,
        taskId,
        durationMs:
      ...
      if (message.includes('AbortError')) {
        ...
        brainSubagentsLog.warn('sub-agent killed', { sessionId, taskId });
      } else {
        record.status = 'failed';
        record.error = message;
        brainSubagentsLog.error('sub-agent failed', {
          sessionId,
          taskId,
          error: message,
        });
  ...
  return {
    ...
    async run(opts) {
      ...
      if (countRunning(sessionId) >= maxConcurrent) {
        brainSubagentsLog.warn('sub-agent rejected', {
          sessionId,
      ...
      brainSubagentsLog.info('sub-agent spawn', {
        sessionId,
        taskId,
        taskType,
        description,
    ...
    async resume(opts) {
      ...
      brainSubagentsLog.info('sub-agent resume', {
        sessionId,
        taskId,
    ...
    async fork(opts) {
      ...
      brainSubagentsLog.info('sub-agent fork', {
        sessionId,
        parentTaskId,
        childId,
        taskType,
