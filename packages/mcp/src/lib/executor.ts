 ...
 *   2. Return a ToolResult with `durationMs` and, on failure, `error`.
 *
 * The raw connector call itself is passed in — this keeps executor
 * decoupled from the registry's routing logic.
 */
/**
 * Flatten a ToolResult's content to a string we can compare across polls.
 ...
 */
function fingerprint(result: ToolResult): string {
  try {
...
export async function executeWithPolicy(
  ...
  const isReady = policy.isReady ?? ((r: ToolResult) => !policy.isQueued(r));
