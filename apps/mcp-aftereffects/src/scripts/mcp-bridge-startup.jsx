...
// Silent startup-script variant. No UI. Runs automatically when AE launches.

// -- Diagnostic: prove the startup script actually loaded --
(function(){
    try {
        var dir = new Folder(Folder.myDocuments.fsName + "/ae-mcp-bridge");
        if (!dir.exists) dir.create();
        var f = new File(dir.fsName + "/startup-loaded.log");
        f.open("w");
...
// -- Startup-script variant: no UI. Auto-runs silently.
// Stubbed UI objects so the rest of the file (which references panel,
// statusText, logText, autoRunCheckbox) keeps working without a panel.
var panel = { update: function(){} };
var statusText = { text: "" };
var logText = { text: "" };
...
// Startup scripts sometimes run in a different scope than where scheduleTask
// later evaluates its string arg — explicitly publish the handler to $.global
// so scheduleTask can always find it.
function startCommandChecker() {
    try {
        $.global.__mcpCheckForCommands = checkForCommands;
        app.scheduleTask("$.global.__mcpCheckForCommands()", checkInterval, true);
        // Heartbeat log so we can tell scheduleTask succeeded
        var f = new File(Folder.myDocuments.fsName + "/ae-mcp-bridge/scheduler-started.log");
        f.open("w");
        f.write("scheduleTask started at " + (new Date()).toString() + "\n");
        f.close();
    } catch (e) {
        var ef = new File(Folder.myDocuments.fsName + "/ae-mcp-bridge/scheduler-error.log");
        ef.open("w");
        ef.write("scheduleTask error: " + e.toString() + "\n");
        ef.close();
    }
}

// Log startup (silent — no panel to write to)
logToPanel("MCP Bridge Startup script loaded");
logToPanel("Command file: " + getCommandFilePath());

// Start the background command checker. No panel needed.
startCommandChecker();
