...
// We install the startup variant (no UI, runs automatically on AE launch).
const sourceScript = path.join(__dirname, 'build', 'scripts', 'mcp-bridge-startup.jsx');
const destinationFolder = isMac
  ? path.join(afterEffectsPath, 'Scripts', 'Startup')
  : path.join(afterEffectsPath, 'Support Files', 'Scripts', 'Startup');
const destinationScript = path.join(destinationFolder, 'mcp-bridge-startup.jsx');
