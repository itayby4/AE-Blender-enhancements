...
export async function generateWithGemini(prompt: string, imageRef?: string): Promise<{ url?: string; status: string; id: string; type: string }> {
  console.log(`[IMAGE-GEN] Calling Nano Banana 2 (Gemini 3.1 Flash Image) with prompt: "${prompt}"`);

  if (imageRef) {
    console.log(`[IMAGE-GEN] Received image reference! Size: ${Math.round(imageRef.length / 1024)}KB`);
  }
  ...
  try {
    ...
    // If we have an image reference (base64 data URL), attach it as inline data
