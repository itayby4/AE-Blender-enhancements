...
class Semaphore {
  ...
  private queue: (() => void)[] = [];
  constructor(concurrency: number) { this.count = concurrency; }
  ...
  const executePipeline = useCallback(async (startNodeId?: string) => {
    ...
    const executeNode = async (executionId: string): Promise<void> => {
      ...
      while (searchQueue.length > 0) {
        ...
        const incomingEdges = allEdges.filter((e: Edge) => e.target === targetId);
        
        for (const edge of incomingEdges) {
          const parentNode = getNodes().find((n: Node) => n.id === edge.source);
      ...
      try {
        ...
        setNodes((nds) => nds.map(n => 
          n.id === executionId ? { ...n, data: { ...n.data, isGenerating: false, error: errorMessage } } : n
        ));
        
        throw new Error(`Pipeline halted due to error in node ${executionId}`);
      } finally {
    ...
    try {
      const executionTasks = Array.from(reachableNodes).map(id => getOrStartNode(id));
      await Promise.all(executionTasks);
      console.log('[Pipeline] Execution fully completed.');
    } catch (e) {
      console.warn('[Pipeline] Execution halted prematurely due to node failure.');
    } finally {
