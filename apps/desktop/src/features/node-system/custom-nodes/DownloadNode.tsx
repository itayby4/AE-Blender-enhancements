...
export function DownloadNode({ data, selected }: { data: any, selected?: boolean }) {
      ...
      for (const edge of incomingEdges) {
        ...
        if (parentNode.type === 'nullNode') {
          searchQueue.push(parentNode.id); // Continue traversal
           ...
           const isImage = url.startsWith('data:image') || parentNode.data.mediaType === 'image';
    ...
    if (mediaToDownload.length > 0) {
        for (let i = 0; i < mediaToDownload.length; i++) {
            ...
            try {
                ...
                const filename = `pipefx_${media.prefix}_${Date.now()}_${i}.${media.ext}`;
  ...
  if (isCompact) {
    return (
      <div onDoubleClick={() => setIsExpanded(true)} className="w-[580px] h-[140px] bg-[#2a2a2a] border-4 border-[#111] rounded-lg shadow-2xl flex items-center justify-center relative hover:bg-[#333] transition-colors cursor-pointer">
        ...
        <span className="text-gray-200 text-5xl font-extrabold tracking-wider px-8 truncate block text-center w-full">{data.label || 'Download'}</span>
