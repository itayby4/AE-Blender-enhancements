...
import { Loader2, Mic, Music, AudioLines, Headphones, Volume2 } from 'lucide-react';
import { Card, CardContent, CardHeader, CardTitle } from '../../../components/ui/card';

export function SoundNode({ data, selected }: { data: any, selected?: boolean }) {
  const SOUND_STYLES: Record<string, { icon: any; borderColor: string; bgColor: string; textColor: string; glowColor: string }> = {
    'elevenlabs-tts':     { icon: Mic,        borderColor: 'border-cyan-400/50 hover:border-cyan-400',       bgColor: 'bg-cyan-400/20',     textColor: 'text-cyan-400',   glowColor: 'shadow-cyan-500/20' },
    'elevenlabs-sfx':     { icon: Music,      borderColor: 'border-orange-400/50 hover:border-orange-400',   bgColor: 'bg-orange-400/20',   textColor: 'text-orange-400', glowColor: 'shadow-orange-500/20' },
    'elevenlabs-sts':     { icon: AudioLines, borderColor: 'border-teal-400/50 hover:border-teal-400',       bgColor: 'bg-teal-400/20',     textColor: 'text-teal-400',   glowColor: 'shadow-teal-500/20' },
    'elevenlabs-isolate': { icon: Headphones, borderColor: 'border-pink-400/50 hover:border-pink-400',       bgColor: 'bg-pink-400/20',     textColor: 'text-pink-400',   glowColor: 'shadow-pink-500/20' },
  };
  ...
  return (
    <div className="relative w-64">
      ...
      <Card className={`shadow-xl bg-card/95 backdrop-blur-md border-2 overflow-hidden transition-all duration-300 hover:shadow-2xl w-full ${style.borderColor} ${isGenerating ? 'ring-2 ring-primary ring-offset-2 ring-offset-background' : ''}`}>
        ...
        <CardContent className="p-0 flex flex-col">
          ...
          <div className="relative w-full bg-black/40 flex items-center justify-center p-4 min-h-[64px] border-b border-border/50">
            {data.error ? (
              <div className="flex flex-col items-center gap-1 text-center w-full">
                ...
                <span className="text-[9px] text-red-400 break-words opacity-80 leading-tight line-clamp-2 font-mono">{data.error}</span>
              </div>
            ) : isGenerating ? (
          ...
          <div className="p-3 bg-muted/5">
            <label className="text-[10px] uppercase font-bold tracking-wider text-muted-foreground/80 pl-0.5 mb-1.5 block">
              {data.model === 'elevenlabs-isolate' ? 'Connect audio input ↑' : 
               data.model === 'elevenlabs-sts' ? 'Connect audio + enter text' : 'Text Input'}
            </label>
            <textarea
              ...
              placeholder={
                data.model === 'elevenlabs-sfx' ? 'Describe the sound effect...' :
                data.model === 'elevenlabs-isolate' ? 'No text needed — connect audio' :
                'Enter text to speak...'
              }
              value={prompt}
