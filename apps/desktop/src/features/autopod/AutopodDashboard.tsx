...
interface AutopodDashboardProps {
  activeApp?: string;
}
    ...
    try {
      ...
      const response = await fetch(
        'http://localhost:3001/api/autopod/discover',
        {
          ...
          body: JSON.stringify({ app_target: target }),
        }
      );
      ...
      let idx = 0;
      for (const src of data.audio_sources || []) {
        if (src.channels === 1) {
          idx++;
          newMappings.push({
            id: `ch_${idx}`,
                  ...
                  mappings.map((mapping) => (
                    ...
                    >
                      ...
                      <div className="flex items-center gap-2 shrink-0">
                        ...
                        <Select
                          value={mapping.cameraId}
                          onValueChange={(v) => {
                            if (v) handleChangeCamera(mapping.id, v);
                          }}
                        >
